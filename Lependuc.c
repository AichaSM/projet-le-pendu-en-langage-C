...
void affichageScore()
{
    ...
    if (s == NULL)
    {
        printf("Vous êtes le premier joueur!! c'est à vous de marquer le meilleur score!!!");
        return;
    }
...
{
    char buffer[1000];//réserver une mémoire de 1000 octets pour la lecture des ligne
...
void main(void)
{
    ...
    {
        int compt = 0;//nombre de tantaive du joueur 
        ...
        {
            printf("\nLe mot à deviner est composé de %d", taille);
            printf(" caractéres et il est sous la forme de %s", hiddenWord);
            ...
            if(isalpha(lettre) != 0){//si le joueur ne propose pas une lettre
                ...
                {
                    printf("\nVous avez déja tenté cette lettre");
                }
            ...
            } else printf("oups!!c'était pas une lettre!!");  
        ...
        {
            printf("Vous avez perdu :( , Le mot était %s",pickedWord);
